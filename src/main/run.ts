...
export const keysStart = (): Promise<any> => {
  let keysPath = binPath('bin/keys')
  if (process.env.KEYS) {
    keysPath = process.env.KEYS
  }
...
export const keysd = (): Promise<any> => {
  ...
  console.log('process.env.KEYSD', process.env.KEYSD)
  if (process.env.KEYSD) {
    return spawnProc(process.env.KEYSD, true)
  }
