...
// InputID: 352ad7066e56ad34168f0bbc83fc72f0be00eee0
...
export const authPasswordChange = (req: AuthPasswordChangeRequest) => {
	return new Promise<AuthPasswordChangeResponse>((resolve, reject) => {
		const reply = 'Keys.AuthPasswordChange-' + replyID()
		ipcRenderer.on(reply, (event, arg) => {
			ipcRenderer.removeAllListeners(reply)
			if (arg.err) {
				console.error('RPC error (Keys.AuthPasswordChange):', arg.err)
				reject(arg.err)
				errHandler(arg.err)
			} else {
				console.log('Keys.AuthPasswordChange')
			}
			resolve(arg.resp)
		})
		ipcRenderer.send('rpc', {service: 'Keys', method: 'AuthPasswordChange', args: req, reply: reply})
	})
}
