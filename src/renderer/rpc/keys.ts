...
// InputID: 7964a47a976c46961d2e9ec3d7e74e4ff820d899
...
export const preferences = (
  ...
  cb: (err: RPCError, resp: PreferencesResponse) => void
) => {
  const reply = 'Keys.Preferences-' + replyID()
  ipcRenderer.on(reply, (event, arg) => {
    ipcRenderer.removeAllListeners(reply)
    if (arg.err) {
      console.error('RPC error (Keys.Preferences):', arg.err)
      errHandler(arg.err)
    } else {
      console.log('RPC (Keys.Preferences) done')
    }
    cb(arg.err, arg.resp)
  })
  console.log('RPC (Keys.Preferences)...')
  ipcRenderer.send('rpc', {service: 'Keys', method: 'Preferences', args: req, reply: reply})
}

export const preferenceSet = (
  req: PreferenceSetRequest,
  cb: (err: RPCError, resp: PreferenceSetResponse) => void
) => {
  const reply = 'Keys.PreferenceSet-' + replyID()
  ipcRenderer.on(reply, (event, arg) => {
    ipcRenderer.removeAllListeners(reply)
    if (arg.err) {
      console.error('RPC error (Keys.PreferenceSet):', arg.err)
      errHandler(arg.err)
    } else {
      console.log('RPC (Keys.PreferenceSet) done')
    }
    cb(arg.err, arg.resp)
  })
  console.log('RPC (Keys.PreferenceSet)...')
  ipcRenderer.send('rpc', {service: 'Keys', method: 'PreferenceSet', args: req, reply: reply})
}
...
export const gitSetup = (
  req: GitSetupRequest,
  cb: (err: RPCError, resp: GitSetupResponse) => void
) => {
  const reply = 'Keys.GitSetup-' + replyID()
  ipcRenderer.on(reply, (event, arg) => {
    ipcRenderer.removeAllListeners(reply)
    if (arg.err) {
      console.error('RPC error (Keys.GitSetup):', arg.err)
      errHandler(arg.err)
    } else {
      console.log('RPC (Keys.GitSetup) done')
    }
    cb(arg.err, arg.resp)
  })
  console.log('RPC (Keys.GitSetup)...')
  ipcRenderer.send('rpc', {service: 'Keys', method: 'GitSetup', args: req, reply: reply})
}
