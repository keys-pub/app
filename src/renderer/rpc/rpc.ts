...
// InputID: 78d6b6ec8e52fea3fb85dfc1db7fbd0cf9f8ba24
...
export const search = (req: SearchRequest, respFn: ((resp: SearchResponse) => void) | void, errFn: ErrFn | void) => async (dispatch: (action: any) => void) => {
  dispatch({
    type: 'KEYS_SEARCH_REQUEST',
		payload: req,
	})
	let cl = await client()
  cl.search(req, (err: RPCError|void, resp: SearchResponse | void) => {
    if (err) {
      if (errFn) {
        console.error(err)
        errFn(err)      
