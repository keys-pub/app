...
type Props = {
  total: number,
  request: (req: Request, (resp: Response) => void, (err: Error) => void) => void,
  rowRender?: (index: number, row: any) => any,
}

class VList extends Component<Props> {
  list: Array<any> = []
  ...
  rowRenderComponents = (index: number, row: React$Element<any>) => {
  ...
  rowRender = (index: number, row: React$Element<any>, styles: any) => {
    return (
    ...
    const fn = (resolve, reject) => {
      ...
      this.props.request(
        ...
        (resp: Response) => {
          ...
          let i = r.startIndex
          for (const row: any of resp.rows) {
            this.list[i++] = row
          }
          resolve()
        },
