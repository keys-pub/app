...
export default class MessagesListView extends PureComponent<Props, State> {
  ...
  rowRenderer = (index: number) => {
    let messageRow: ?MessageRow = this.list[index]
  ...
  selectMessage = (r: MessageRow, index: number) => {
    let selectedRow: ?MessageRow = this.list[this.selectedIndex]
  ...
  loadRows = (index: number) => {
    ...
    this.props.dispatch(
      messages(req, (resp: MessagesResponse) => {
        ...
        const sp: ?ScrollPosition = this.scrollPositions.get(this.props.kid)
